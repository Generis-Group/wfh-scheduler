...
describe("access guards", () => {
  ...
  it("blocks temporary-password users except on allowed routes", async () => {
    mockedAuth.mockResolvedValue(
      session("EMPLOYEE", { mustChangePassword: true }),
    );

    await expect(requireSession()).rejects.toMatchObject({ status: 403 });
    await expect(
      requireSession({ allowPasswordChangeRequired: true }),
  ...
  it("allows owners and admins through the synchronous user guard", () => {
    ...
    expect(
      canAccessUser(
        session("EMPLOYEE", { roles: ["EMPLOYEE", "ADMIN"] })!,
        "employee-1",
      ),
    ).toBe(true);
  ...
  it("allows only employee owners to mutate reports", () => {
    expect(canMutateReport(session("EMPLOYEE")!, { userId: "user-1" })).toBe(
      true,
    );
    expect(
      canMutateReport(session("ADMIN", { roles: ["EMPLOYEE", "ADMIN"] })!, {
        userId: "user-1",
      }),
    ).toBe(true);
    expect(
      canMutateReport(session("EMPLOYEE")!, { userId: "employee-1" }),
    ).toBe(false);
    expect(
      canMutateReport(session("REVIEWER")!, { userId: "employee-1" }),
    ).toBe(false);
    expect(canMutateReport(session("ADMIN")!, { userId: "employee-1" })).toBe(
      false,
    );
  });
  ...
  it("enforces role allow lists", async () => {
    ...
    await expect(requireRole(["REVIEWER", "ADMIN"])).rejects.toBeInstanceOf(
      HttpError,
    );
  });
  ...
  it("does not let admin-only users review reports", async () => {
    mockCanReviewEmployee.mockResolvedValue(true);

    await expect(
      assertCanReviewReport(session("ADMIN")!, { userId: "employee-1" }),
    ).rejects.toBeInstanceOf(HttpError);
    expect(mockCanReviewEmployee).not.toHaveBeenCalled();
  });
});
